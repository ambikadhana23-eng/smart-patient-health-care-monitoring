// Smart Healthcare System - Frontend Application Logic
// Handles routing, auth, API calls, and all UI interactions
...
async function checkDueReminders() {
  ...
  try {
    ...
    if (data.due && data.due.length > 0) {
      data.due.forEach(r => {
        toast('⏰ Medicine Reminder', `Time to take ${r.medicine_name} ${r.dosage || ''}`, 'warning');
      });
    }
  } catch (err) {}
}
...
async function loadPatientDashboard() {
  ...
  if (!state.reminderPollTimer) {
    checkDueReminders();
    state.reminderPollTimer = setInterval(checkDueReminders, 60000);
  }
}
